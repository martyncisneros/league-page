<script>
  ...
  const checkIfDeserves = (awardRosterID, userRosterID, year) => {
    ...
    return checkIfManagerReceivedAward(
      leagueTeamManagers,
      awardRosterID,
      year,
      managerID
  ...
  const checkIfDeservesWithManagerID = (recordManagerID, userRosterID) => {
    ...
    for (const year in leagueTeamManagers.teamManagersMap) {
      for (const rosterID in leagueTeamManagers.teamManagersMap[year]) {
        if (
          leagueTeamManagers.teamManagersMap[year][rosterID].managers.indexOf(
            recordManagerID
          ) > -1
        ) {
    ...
    {#each displayAwards as award}
      <div class="award">
        ...
        <div class="awardLabel">
          {award.type == "award" ? `${award.year} ` : ""}{computeAward(
            award.award
          )}{award.former ? "*" : ""}
        </div>
        {#if award.extraInfo}
          <div class="subText">
            {award.year ? `${award.year} ` : ""}{award.week
              ? `Week ${award.week} `
              : ""}{award.year || award.week
              ? " - "
              : ""}{award.extraInfo}{award.wins ? " Wins" : ""}{award.iq
              ? "%"
              : ""}{!award.wins && !award.iq ? "pts" : ""}
          </div>
        {/if}
      </div>
    {:else}
